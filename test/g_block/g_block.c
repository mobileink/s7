...
#include "s7.h"
#include "bazel.h"
#include "libg_block.h"

int run_g_block(s7_scheme *s7)
{
    /* printf("run_g_block\n"); */
    /* s7_pointer newpath = s7_load_path(s7); */
    /* printf("load_path result: %s\n", s7_object_to_c_string(s7, newpath)); */

    /* g_block.scm, as a runtime dep, must be listed in 'data' attrib of
       bazel build rule */
    static char *load_script = "g_block.scm";
